...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    //衝突の効果音。
    let sound = SKAction.playSoundFileNamed("se.mp3", waitForCompletion: false)
    let contup = SKAction.playSoundFileNamed("up.mp3", waitForCompletion: false)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if scrollNode.speed > 0 { // scrollNodeのspeedプロパティが0より大きい時）だけ鳥が羽ばたくようにする
        // 鳥の速度をゼロにする
        bird.physicsBody?.velocity = CGVector.zero

        // 鳥に縦方向の力を与える
        bird.physicsBody?.applyImpulse(CGVector(dx: 0, dy: 15))
        } else if bird.speed == 0 { // ゲームオーバー状態（scrollNodeのspeedプロパティが0）で鳥の回転アクションが停止（birdのspeedプロパティが0）になっていたら、restart()メソッドを呼び出してゲームを再開
            restart()
        }
    }
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        //bodyAの名前が"item"のとき。
        if contact.bodyA.node?.name == "score" {
            // スコア用の物体と衝突した
            print("ScoreUp")
            score += 1
        ...
        //bodyAの名前が"item"のとき。
        if contact.bodyA.node?.name == "under" {
        ...
        //bodyAの名前が"item"のとき。
        if contact.bodyA.node?.name == "upper" {
    ...
    func setupItem() {
        ...
        // 画像を読み込む
        let itemTexture = SKTexture(imageNamed: "poop")
        ...
        let createItemAnimation = SKAction.run({
            ...
            // 0〜random_y_rangeまでのランダム値を生成
            let random_y = CGFloat.random(in: 0..<150)
            // Y軸の下限にランダムな値を足して、下の壁のY座標を決定
            ...
            // スプライトの表示する位置を指定する
            item.position = CGPoint(x: self.frame.size.width + wallTexture.size().width * 3, y: item_y)
            item.zPosition = -90
            item.setScale(3.0)
        ...
        // 次の作成までの時間待ちのアクションを作成
        let waitAnimation = SKAction.wait(forDuration: 2)
            
        // 作成->時間待ち->作成を無限に繰り返すアクション
